...
const mapToAxisInstruction = instructionString => {
  const direction = instructionString.slice(0,1)
  const magnitude = parseInt(instructionString.slice(1))
  return {
    axis: ['L', 'R'].includes(direction) ? X : Y,
    operator: ['L', 'D'].includes(direction) ? subtract : add,
    magnitude
  }
}

const mapToCoordinates = (instructions, coordinatesList) => {
  const remainingInstructions = instructions.slice(1)
  const thisInstruction = instructions[0]
  
  const newCoordinate = Object.assign({}, coordinatesList[coordinatesList.length - 1])
  
  const newCoordinates = Array(thisInstruction.magnitude).fill().map((item, index) => {
    const point = Object.assign({}, newCoordinate)
    point[thisInstruction.axis] = thisInstruction.operator(point[thisInstruction.axis], index + 1)
    return point
  })
  
  const newCoordinateList = coordinatesList.concat(newCoordinates)
  if (remainingInstructions.length > 0) {
    return mapToCoordinates(remainingInstructions, newCoordinateList)  
  }
  return newCoordinateList
}

const oneInstructions = one.split(',').map(mapToAxisInstruction)
console.log('mapped one to instructions')
const twoInstructions = two.split(',').map(mapToAxisInstruction)
console.log('mapped two to instructions')
  
const oneCoordinates = mapToCoordinates(oneInstructions, [initialCoordinates])
console.log('mapped one to coordinates', oneCoordinates.length)
const twoCoordinates = mapToCoordinates(twoInstructions, [initialCoordinates])
console.log('mapped two to coordinates', twoCoordinates.length)

const distances = oneCoordinates
  .filter(o => twoCoordinates.filter(t => t.x === o.x && t.y === o.y).length > 0)
  .filter(o => o.x !== 0 && o.y !== 0)
  .map(o => {
    const indexOfOne = oneCoordinates.indexOf(o)
    const indexOfTwo = twoCoordinates.indexOf(twoCoordinates.find(t => t.x === o.x && t.y === o.y))
    return {
      ...o, 
      distance: Math.abs(o.x) + Math.abs(o.y),
      stepsInOne: indexOfOne,
      stepsInTwo: indexOfTwo,
      totalSteps: indexOfOne + indexOfTwo
    }
  })
